...
class Service:
    ...
    def floyd_warshall(self, start: int, end: int):
        """Floyd-Warshall algorithm.
        start: int - the starting vertex
        end: int - the ending vertex
        Returns: list - the lowest cost walk between the given vertices"""
        if start not in self.graph.get_vertices() or end not in self.graph.get_vertices():
            raise ValueError("Vertices not in the graph")
        n = self.graph.get_num_vertices()
        dist = [[float("inf") for _ in range(n)] for _ in range(n)]
        nextV = [[-1 for _ in range(n)] for _ in range(n)]
        for x, y in self.graph.get_edges():
            dist[x][y] = self.graph.get_cost_of_edge(x, y)
            nextV[x][y] = y
        for i in range(n):
            dist[i][i] = 0
        for k in range(n):
